...
int benchmark(NanoDet& detector)
{
    ...
    double time_avg = 0;
    std::vector<float> inputTensorValues(height * width * 3);
    cv::Mat srcResize = cv::Mat(cv::Size(width, height), CV_8UC3, cv::Scalar(1));
    ...
    for (int i = 0; i < warm_up + loop_num; i++) {
        ...
        auto outputTensor = detector.detect(srcResize, 0.4, 0.5);
...
int main(int argc, char** argv)
{
    ...
    NanoDet detector = NanoDet("./nanodet.onnx", 4);
